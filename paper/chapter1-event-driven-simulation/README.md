...
| 小节 | 标题 | 字数 |
|------|------|------|
| 1.1 | [事件驱动模拟的概念与优势](./1.1-基本概念.md) | 4000 |
| 1.2 | [事件驱动机制的设计与实现](./1.2-事件驱动机制的设计与实现.md) | 5000 |
| 1.3 | [事件驱动执行的性能优化](./1.3-事件驱动执行的性能优化.md) | 4500 |
| 1.4 | [时间同步与多时钟域](./1.4-时间同步与多时钟域.md) | 4500 |
| 1.5 | [性能监测与轨迹捕捉](./1.5-性能监测与轨迹捕捉.md) | 4000 |
...
4. **事件优先级如何设置?**
   - 答: Connection < Component < Combination的三层优先级结构
   - 详见: 1.2, 1.4

5. **如何处理多时钟域系统?**
   - 答: 事件驱动天然支持多时钟域，无需额外同步
   - 详见: 1.4
