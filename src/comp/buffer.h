 ...
 * Inherits from Component. Supports:
...
class Buffer : public Architecture::Component {
 public:
   ...
   *
   * Creates read and write ports based on configuration
   */
  void initialize() override {
    ...
    for (uint32_t i = 0; i < params_.num_write_ports; ++i) {
      std::stringstream ss;
      ss << "write_port_" << i;
      addPort(ss.str(), Architecture::PortDirection::INPUT);
    }
    ...
    for (uint32_t i = 0; i < params_.num_read_ports; ++i) {
      std::stringstream ss;
      ss << "read_port_" << i;
      addPort(ss.str(), Architecture::PortDirection::OUTPUT);
    }

    // Create control ports
    addPort("write_enable", Architecture::PortDirection::INPUT);
    addPort("read_enable", Architecture::PortDirection::INPUT);
    addPort("full_flag", Architecture::PortDirection::OUTPUT);
    addPort("empty_flag", Architecture::PortDirection::OUTPUT);
    addPort("occupancy", Architecture::PortDirection::OUTPUT);
  }
