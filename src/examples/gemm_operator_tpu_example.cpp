...
int main() {
  ...
  GEMMOperator<int> gemm_op("GEMM_TPU");
  ...
  Tensor<int> A1(TensorShape{4, 4});
  Tensor<int> B1(TensorShape{4, 4});
  ...
  const Tensor<int>& C1 = gemm_op.getOutput();
  std::cout << "\nResult Matrix C:\n";
  C1.print("C");
  ...
  Tensor<int> A2(TensorShape{8, 8});
  Tensor<int> B2(TensorShape{8, 8});

  // Simple patterns for testing
  for (size_t i = 0; i < 8; ++i) {
    for (size_t j = 0; j < 8; ++j) {
      A2.at(i, j) = (i + 1);  // Row-based values
      B2.at(i, j) = (j + 1);  // Column-based values
    }
  }
  ...
  const Tensor<int>& C2 = gemm_op.getOutput();
  ...
  Tensor<int> A3(TensorShape{16, 16});
  Tensor<int> B3(TensorShape{16, 16});
  ...
  Tensor<int> C_tpu = gemm_op.getOutput();
  ...
  for (size_t i = 0; i < 16; ++i) {
    for (size_t j = 0; j < 16; ++j) {
      if (C_tpu.at(i, j) != C_cpu_naive.at(i, j)) match_naive = false;
      if (C_tpu.at(i, j) != C_cpu_tiled.at(i, j)) match_tiled = false;
    }
  }
  ...
  Tensor<int> A4(TensorShape{4, 8});  // 4x8
  Tensor<int> B4(TensorShape{8, 4});  // 8x4
  // Result will be 4x4
  ...
  const Tensor<int>& C4 = gemm_op.getOutput();
  C4.print("Result");
