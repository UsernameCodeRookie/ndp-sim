...
TEST_F(BruTest, Initialization) {
  auto bru =
      std::make_shared<Architecture::BranchUnit>("test_bru", *scheduler, 1);
  bru->start();
...
TEST_F(BruTest, BranchEqual) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x100, Architecture::BruOp::BEQ, 5, 5);
  ...
  auto result = std::dynamic_pointer_cast<Architecture::BruResultPacket>(
      output_port->read());
...
TEST_F(BruTest, BranchNotEqual) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x100, Architecture::BruOp::BEQ, 5, 3);
  input_port->write(cmd);
...
TEST_F(BruTest, BranchNotEqualOp) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x200, Architecture::BruOp::BNE, 5, 3);
  input_port->write(cmd);
...
TEST_F(BruTest, BranchLessThanSigned) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x150, Architecture::BruOp::BLT, 0xFFFFFFFB, 3);  // -5 as unsigned
  input_port->write(cmd);
...
TEST_F(BruTest, BranchLessThanUnsigned) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x300, Architecture::BruOp::BLTU, 5, 10);
  input_port->write(cmd);
...
TEST_F(BruTest, JumpAndLink) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x100, 0x400, Architecture::BruOp::JAL, 0, 0, 1);
  input_port->write(cmd);
...
TEST_F(BruTest, JumpAndLinkRegister) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x200, 0, Architecture::BruOp::JALR, 0x501, 0, 1);
  input_port->write(cmd);
...
TEST_F(BruTest, MultipleBranches) {
  ...
  auto cmd1 = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x100, Architecture::BruOp::BEQ, 7, 7);
  input_port->write(cmd1);
  for (int i = 0; i < 4; i++) {
    bru->tick();
  }
  auto result1 = std::dynamic_pointer_cast<Architecture::BruResultPacket>(
      output_port->read());
  EXPECT_TRUE(result1->taken);

  // Test 2: BNE with condition true
  auto cmd2 = std::make_shared<Architecture::BruCommandPacket>(
      0x100, 0x200, Architecture::BruOp::BNE, 5, 3);
  input_port->write(cmd2);
  for (int i = 0; i < 4; i++) {
    bru->tick();
  }
  auto result2 = std::dynamic_pointer_cast<Architecture::BruResultPacket>(
      output_port->read());
  EXPECT_TRUE(result2->taken);

  // Test 3: JAL
  auto cmd3 = std::make_shared<Architecture::BruCommandPacket>(
      0x200, 0x300, Architecture::BruOp::JAL, 0, 0, 1);
  input_port->write(cmd3);
  for (int i = 0; i < 4; i++) {
    bru->tick();
  }
  auto result3 = std::dynamic_pointer_cast<Architecture::BruResultPacket>(
      output_port->read());
...
TEST_F(BruTest, BranchGreaterEqualSigned) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x250, Architecture::BruOp::BGE, 10, 5);
  input_port->write(cmd);
...
TEST_F(BruTest, BranchGreaterEqualUnsigned) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x0, 0x350, Architecture::BruOp::BGEU, 10, 10);
  input_port->write(cmd);
...
TEST_F(BruTest, SystemOperations) {
  ...
  auto cmd = std::make_shared<Architecture::BruCommandPacket>(
      0x100, 0x0, Architecture::BruOp::ECALL);
  input_port->write(cmd);
...
TEST_F(BruTest, EventDrivenExecution) {
  ...
  auto bru = std::make_shared<Architecture::BranchUnit>("test_bru_event",
                                                        *scheduler, 2);
  bru->start();
